...
/**
 * Extract Express routes from TypeScript/JavaScript source files using ts-morph AST parsing.
 *
...
export async function extractExpressRoutes(
  ...
  for (const sourceFile of project.getSourceFiles()) {
    ...
    // Build a map of router variable name → mount prefix from app.use() calls.
    // e.g. app.use("/api", router) → { router: "/api" }
    const mountPrefixes = new Map<string, string>();

    const callExpressions = sourceFile.getDescendantsOfKind(SyntaxKind.CallExpression);

    // First pass: collect app.use("/prefix", routerVar) mounts.
    for (const callExpr of callExpressions) {
      ...
      // Second arg should be an identifier (the router variable).
      if (secondArg.getKind() === SyntaxKind.Identifier) {
        const routerVarName = secondArg.asKindOrThrow(SyntaxKind.Identifier).getText();
        mountPrefixes.set(routerVarName, prefix);
      }
    }

    // Second pass: collect router.METHOD("/path", handler) calls.
    for (const callExpr of callExpressions) {
      const expr = callExpr.getExpression();
      if (expr.getKind() !== SyntaxKind.PropertyAccessExpression) continue;
      ...
      // Determine if the receiver is a known router variable, and look up its mount prefix.
      ...
      const params = (fullPath.match(/:([^/]+)/g) ?? []).map((p) => p.slice(1));

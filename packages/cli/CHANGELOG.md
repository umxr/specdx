...
- 1b3f22c: Lint AGENTS.md and CLAUDE.md
...
- de0f61e: Fix `specdx lint <path>` reporting a clean pass when the path matched no spec
  ...
  the whole suite, and the path argument filtered _diagnostics_ rather than the
  ...
  list _before_ linting, so a single-file lint could not see the rest of the

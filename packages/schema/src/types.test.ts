...
import type {
  BaseSpec, PrdSpec, UserStorySpec, AdrSpec, TechnicalDesignSpec,
  TestPlanSpec, ApiContractSpec, SpecType, SpecStatus, SpecReference,
  SdxConfig, SpecEntry,
} from "./types.js";
...
describe("types", () => {
  it("exports SPEC_TYPES constant matching the type union", () => {
    expect(SPEC_TYPES).toEqual(["prd", "technical-design", "user-story", "test-plan", "adr", "api-contract"]);
  });
  ...
  it("UserStorySpec requires story_id, priority, estimate", () => {
    const story: UserStorySpec = {
      id: "s-001", type: "user-story", title: "Login", status: "draft",
      version: "1.0", created: "2026-01-01", authors: ["dev"],
...
describe("sections", () => {
  ...
  it("defines required sections for all spec types", () => {
    for (const type of ["prd", "technical-design", "user-story", "test-plan", "adr", "api-contract"] as const) {
